...
namespace PlantUML
{
    public class UMLComponentDiagramParser
    {
        ...
        private static Regex _packageRegex = new Regex("^\\s*(?<type>package|frame|node|cloud|database|node|folder) +\\\"*(?<name>[\\w ]+)*\\\"* *\\{", RegexOptions.Compiled | RegexOptions.IgnoreCase);
